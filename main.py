...
class TitanLive:
    ...
    async def _execute_tool_by_name(self, name: str, raw_args: dict, call_id: str = "brain") -> str:
        ...
        # the user can still steer or cancel a running worker mid-flight, AND
        # answer_worker_question — a worker can be sitting inside ask_boss()
        # blocked on a real asyncio.Future waiting for exactly this call. If
        # it's caught by this guard instead of reaching task_workers
        # .answer_question(), the future never resolves and the worker just
        # hangs silently for its full 240s timeout with no visible sign
        # anything is wrong — this is what "goes silent after asking a
        # question" was: the delegation guard was eating the boss's answer
        # before it ever reached the waiting worker.
        if (
            call_id != "brain"
            and self._live_turn_delegated
            and name not in {
                "task_worker_status", "send_task_worker_message", "interrupt_task_worker",
                "answer_worker_question", "set_titan_microphone", "shutdown_titan",
            }
        ):
